...
###
class RearTruncateDataset(BaseWrapperDataset):
    """Truncate a sequence by returning the rear truncation_length tokens"""
    ...
    def __getitem__(self, index):
        ...
        item_len = item.size(0)
        if item_len > self.truncation_length:
            item = item[(item_len-self.truncation_length):]
        return item
